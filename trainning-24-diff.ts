/**
 * @params {array} targetArray
 * @params {array} compareArray
 * @params {array} originalArray
 * @returns {object} 중복된 요소와, 바뀔 요소를 담은 객체
 *
 * git의 diff기능처럼 두 값(배열)을 비교하여, 중복되는 요소가 있을 경우
...
function diffFinder(
  ...
  compareArray: Array<string>,
  originalArray: Array<string>
) {
  ...
  // null데이터 타입을 명시하는 것이 맞습니다.
  // 하지만 아래의 실행에 의해 string으로 변환될 예정이기에 에러가 발생할 것이 확정입니다.
  // 타입 명시, 타입 표기도 약간의 변용 여지를 남겨주기위해 여러 타입을 명시하는 것을
  // 합집합타입, 유니온타입이라고  합니다.(union type)

  targetArray.forEach((element, index) => {
    if (compareArray.includes(element) === true) {
      duplicatedElement = element; // 중복되어있는 요소를 미리 만들어둔 변수에 할당
